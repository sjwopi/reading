function PopupWithFavorite({ card, onClose, onCloseOverlay }) {
  return (
    <div className={`popup ` + (card !== null && ' popup_opened popup-favorite_opened')} onClick={onCloseOverlay}>
      <div className="popup-favorite__container">
        ...
        />
        <button className="popup-favorite__save" onClick={onClose} >Прочтено</button>
        <button className="popup__close-btn popup__close-btn_favorite" onClick={onClose} ></button>
      </div>
    </div>
  );
}
